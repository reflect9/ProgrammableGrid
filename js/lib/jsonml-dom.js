			...
			case 11: // documentFragment
				...
				for (i=0; attr && i<attr.length; i++) {
					if (attr[i].specified) {
						...
						} else if ('string' === typeof attr[i].value) {	// convert relative to absolute path
							if(attr[i].name=="href" && attr[i].value.indexOf("http")==-1) {
								props[attr[i].name] = elem.protocol + "//"+ elem.host + attr[i].value;
							} else {
