...
function init() {
	...
	chrome.browserAction.onClicked.addListener(function() {
		chrome.tabs.getSelected(null, function(tab) {
			...
			chrome.tabs.sendMessage(tab.id, {action: "openWorksheet", program:program, tab:tabs[tab.id]},function() {});
		});
	});
		...
		function(request, sender, callback) {
			...
			if(request.action == "reportOnLoad") {
				...
				if(tabs[sender.tab.id]) {
					...
					callback({action:"openWorksheet",program:null, tab:tabs[sender.tab.id]});
				}
			}
			// 
